...
目前 stockAgent 專案已完成資料面板、walk-forward 訓練、Transformer portfolio model、成本感知張量回測、台股漲跌停買賣遮罩、最小交易權重門檻、整數股數稽核輸出與可解釋性基礎。現股當沖、融資、融券、券源與保證金等台股交易制度尚未完整整合至訓練與回測，將作為本論文後續實作重點之一。
...
| 最小交易權重 | `min_trade_weight: 0.005` | 權重絕對值低於 0.5% 者視為噪音，不建立或調整部位 |
...
1. 已完成基礎：市場權杖 Transformer、long-short 權重、成本感知回測、漲跌停遮罩、最小權重門檻與整數股數稽核。
...
5. 最小權重門檻：基準設定為 `min_trade_weight = 0.005`。模型產生之目標權重若絕對值小於 0.5%，視為不可執行或不值得交易的微小訊號，於回測與整數股數稽核中歸零。
...
第二，預測準確不等於交易可行。模型可能每天給出大量極小權重、過度換手或集中於漲停無法買進的股票。若回測只看連續權重與理想成交，績效容易高估。專案新增的 `min_trade_weight` 正是為了處理這類問題：權重低於門檻的微小訊號不交易，讓研究更接近資金有限與交易成本存在的情境。
...
目前基準設定為 $\theta = 0.005$。此門檻可降低小額噪音交易、減少交易筆數與整數股數轉換誤差，但也可能犧牲分散化與弱訊號。因此本研究將進行 $\theta \in \{0, 0.001, 0.0025, 0.005, 0.01\}$ 的敏感度分析。
