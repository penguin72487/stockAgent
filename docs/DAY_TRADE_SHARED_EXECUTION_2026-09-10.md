...
2026-09-18 使用者確認：即時 Shioaji 模擬委託不使用本地 L1 深度、
分鐘成交量或參與率裁切；券商 `StockDeal` 回報的股數與價格就是該模擬
委託的成交證據。`place_order` 回傳的 `Filled`、委託已受理或市價委託
本身都不是完整成交證據；若只回報部分成交，就只記該部分，其餘維持
未成交或待對帳。這不回溯改寫既有本地 paper 帳本，也不改變歷史回測
在沒有券商回報時的容量假設。正式切換仍需獨立處理舊庫存、收據消費、
模擬環境實測及網頁揭露。
