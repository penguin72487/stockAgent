...
`artifacts/markets/tw_minute_dual_5090_daytrade_frequency_only_v4`。這份設定以
...
同一個 `projection_l1` 投資組合頭（`sum(abs(w)) <= 1`，未使用曝險即為 cash）、
每根 KBar 最多參與 50% 成交量、使用正常手續費與當沖稅，且不加額外滑價、單筆
金額上限或單一標的權重上限。模型先分配股票與 cash，再套逐日精確放空資格、
