...
tx_scheduler="${SHIOAJI_FUTURES_HISTORY_RUN_ROOT:-artifacts/data_repair/shioaji_futures_history}/scheduler.json"
...
PY
      ...
      if (( pending > 0 )); then
        # One login is shared by both history runners.  Give a recently
        # lock-blocked TX batch one complete 60-second retry interval before
        # this runner takes the next batch; otherwise keep the fast 30s path.
        tx_waiting="$(history_recent_login_waiter "$tx_scheduler")"
        if [[ "$tx_waiting" == 1 ]]; then history_wait 75 yield_to_waiting_tx_history;
        else history_wait 30 next_repair_batch; fi
