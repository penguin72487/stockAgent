...
# Keep the exact immutable source snapshot used by the completed v12 control
# materialized for the full suite.  The spec additionally pins the panel-cache
# variant/generation/source hash and fails closed on any mismatch.
...
run_fintech_python scripts/run_tw_day_trade_ablation.py \
  --spec configs/ablations/tw_day_trade_hybrid_minute_v12_temporal_basis_checkpoint_ablation_v1.yaml \
