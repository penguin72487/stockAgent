...
service_name="stockagent-tw-public-feature-reconcile.service"
timer_name="stockagent-tw-public-feature-reconcile.timer"
...
if (( EUID != 0 )); then
  echo "[tw-public-feature-reconcile] root privileges are required" >&2
  exit 2
fi
if [[ ! -f "$service_template" || ! -f "$timer_template" ]]; then
  echo "[tw-public-feature-reconcile] unit templates are missing" >&2
  exit 2
fi
...
echo "[tw-public-feature-reconcile] timer=$timer_name state=$(systemctl is-active "$timer_name")"
