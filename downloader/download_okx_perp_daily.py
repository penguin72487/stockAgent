...
def _load_logical_existing_candle_info(path: Path) -> ExistingCandleInfo:
    base = _load_existing_candle_info(path)
...
def _download_symbol_1m(
    ...
    if existing_info is not None and existing_info.rows > 0:
        if tail_only:
            ...
            logical = _load_logical_existing_candle_info(output_path)
            return DownloadResult(
                asset_class="crypto_okx_perp",
                code=record.code,
                okx_symbol=record.okx_symbol,
