...
try:
    from cuml.linear_model import ElasticNet as CuMLElasticNet
...
def _build_estimator(config: ExperimentConfig, model_name: str, use_gpu: bool):
    if use_gpu and model_name == "ridge" and CuMLRidge is not None:
        return CuMLRidge(
            alpha=float(config.training.ridge_alpha),
            fit_intercept=bool(config.training.ridge_fit_intercept),
        )
    if use_gpu and model_name == "elasticnet" and CuMLElasticNet is not None:
        return CuMLElasticNet(
            alpha=float(config.training.elasticnet_alpha),
            l1_ratio=float(config.training.elasticnet_l1_ratio),
            fit_intercept=bool(config.training.elasticnet_fit_intercept),
            max_iter=int(config.training.elasticnet_max_iter),
            tol=float(config.training.elasticnet_tol),
        )
    ...
    try:
        ...
        raise ImportError(
            "scikit-learn is required for Ridge/ElasticNet. Install with: pip install scikit-learn"
        ) from exc
    ...
    raise ValueError(f"Unsupported linear model_name: {model_name}")


def _predict_scores(estimator, x_eval, out_shape: tuple[int, int], use_gpu: bool):
    raw = estimator.predict(x_eval)
    if use_gpu:
        return cp.asarray(raw, dtype=cp.float32).reshape(out_shape)
    return np.asarray(raw, dtype=np.float32).reshape(out_shape)
...
def run_training_linear(
    ...
    if model_name not in {"ridge", "elasticnet"}:
        raise ValueError(f"run_training_linear only supports ridge/elasticnet, got {model_name}")

    requested_gpu = _is_gpu_enabled(config)
    use_gpu = requested_gpu
    if requested_gpu and model_name == "ridge" and CuMLRidge is None:
        print("[Linear] cuML Ridge unavailable, fallback to CPU scikit-learn Ridge.")
        use_gpu = False
    if requested_gpu and model_name == "elasticnet" and CuMLElasticNet is None:
        print("[Linear] cuML ElasticNet unavailable, fallback to CPU scikit-learn ElasticNet.")
        use_gpu = False
    ...
    for fold in fold_list:
        print(
            f"[{model_name}] Fold {fold.fold_id}: train={fold.train_years} "
        ...
        estimator = _build_estimator(config, model_name, use_gpu)
        estimator.fit(x_train, y_train)

        val_shape = tuple(val_view["future_returns"].shape)
        test_shape = tuple(test_view["future_returns"].shape)
        val_scores = _predict_scores(estimator, val_view["x_flat"], val_shape, use_gpu)
        test_scores = _predict_scores(estimator, test_view["x_flat"], test_shape, use_gpu)
        ...
        with (fold_dir / f"{model_name}_model.json").open("w", encoding="utf-8") as handle:
            json.dump(model_payload, handle, indent=2, default=str)
