...
def test_portfolio_output_mode_gated_l1_scales_identity_l1_weights() -> None:
    device = _device()
    model = _make_model(
        attention_mode="market_token",
        portfolio_mode="long_short",
        portfolio_activation="tanh",
        portfolio_output_mode="adaptive_gross_l1",
        portfolio_gross_gate_min=0.10,
        portfolio_gross_gate_max=0.80,
        portfolio_gross_gate_init=0.35,
    ).eval()
    x = torch.randn(2, 6, 13, 11, device=device)
    mask = torch.ones(2, 13, dtype=torch.bool, device=device)
    mask[1, 10:] = False
    ...
    base = masked_activation_l1_weights(
        aux["centered_score_logits"],
        mask,
        long_only=False,
        activation="identity",
    )
    gate = aux["portfolio_gross_gate"]
    expected = base * gate.unsqueeze(1)
    gross = weights.abs().sum(dim=1)

    assert model.portfolio_output_mode == "gated_l1"
    assert torch.allclose(weights, expected, atol=1e-6, rtol=1e-6)
    assert torch.allclose(gate, torch.full_like(gate, 0.35), atol=1e-5, rtol=1e-5)
    assert torch.allclose(aux["portfolio_gross_exposure"], gross, atol=1e-6, rtol=1e-6)
    assert torch.all(gross <= 0.80 + 1e-6)
    assert torch.all(gross >= 0.10 - 1e-6)
    assert weights[1, 10:].abs().max().item() < 1e-6


def test_portfolio_output_mode_gated_net_l1_controls_net_exposure() -> None:
    device = _device()
    model = _make_model(
        attention_mode="market_token",
        portfolio_mode="long_short",
        portfolio_output_mode="adaptive_gross_net_l1",
        portfolio_gross_gate_min=0.10,
        portfolio_gross_gate_max=0.90,
        portfolio_gross_gate_init=0.60,
        portfolio_net_gate_max=0.80,
        portfolio_net_gate_init=0.40,
    ).eval()
    x = torch.randn(2, 6, 13, 11, device=device)
    mask = torch.ones(2, 13, dtype=torch.bool, device=device)
    mask[1, 10:] = False
    ...
    logits = aux["centered_score_logits"].masked_fill(~mask, 0.0)
    long_probs = model._masked_probability(logits, mask)
    short_probs = model._masked_probability(-logits, mask)
    gross_gate = aux["portfolio_gross_gate"]
    net_gate = aux["portfolio_net_gate"]
    long_fraction = aux["portfolio_long_gross_fraction"]
    short_fraction = aux["portfolio_short_gross_fraction"]
    expected = (
        long_fraction.unsqueeze(1) * long_probs
        - short_fraction.unsqueeze(1) * short_probs
    ) * gross_gate.unsqueeze(1)

    assert model.portfolio_output_mode == "gated_net_l1"
    assert torch.allclose(weights, expected, atol=1e-6, rtol=1e-6)
    assert torch.allclose(gross_gate, torch.full_like(gross_gate, 0.60), atol=1e-5, rtol=1e-5)
    assert torch.allclose(net_gate, torch.full_like(net_gate, 0.40), atol=1e-5, rtol=1e-5)
    assert torch.allclose(long_fraction, torch.full_like(long_fraction, 0.70), atol=1e-5, rtol=1e-5)
    assert torch.allclose(short_fraction, torch.full_like(short_fraction, 0.30), atol=1e-5, rtol=1e-5)
    assert torch.allclose(aux["portfolio_net_exposure"], weights.sum(dim=1), atol=1e-6, rtol=1e-6)
    assert torch.allclose(
        aux["portfolio_net_exposure"],
        gross_gate * net_gate,
        atol=1e-6,
        rtol=1e-6,
    )
    assert torch.all(aux["portfolio_gross_exposure"] <= gross_gate + 1e-6)
    assert weights[1, 10:].abs().max().item() < 1e-6
