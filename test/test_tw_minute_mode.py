...
from stockagent.training.minute import (
    ...
    _run_day_batch,
    _stateful_proximal_target_weights,
    run_minute_training,
)
...
def _completed_previous_minute_session() -> pl.DataFrame:
    raw = pl.DataFrame(
        {
            "ts": [
                datetime.fromisoformat("2026-01-02 09:01:00"),
                datetime.fromisoformat("2026-01-02 13:30:00"),
            ],
            "symbol": ["2330", "2330"],
            "Open": [100.0, 101.0],
            "High": [102.0, 104.0],
            "Low": [99.0, 100.0],
            "Close": [101.0, 103.0],
            "volume_shares": [10.0, 20.0],
            "source_volume_unit_valid": [True, True],
            "session_exit_valid": [False, True],
        }
    )
    return summarize_minute_sessions_for_next_day(raw)


def _developing_current_session(*, future_high: float = 110.0) -> pl.DataFrame:
    return pl.DataFrame(
        {
            "ts": [
                datetime.fromisoformat("2026-01-05 09:01:00"),
                datetime.fromisoformat("2026-01-05 09:02:00"),
                datetime.fromisoformat("2026-01-05 09:03:00"),
            ],
            "symbol": ["2330"] * 3,
            "Open": [104.0, 105.0, 107.0],
            "High": [105.0, 107.0, future_high],
            "Low": [103.0, 104.0, 106.0],
            "Close": [105.0, 106.0, 109.0],
            # The schema-4 receipt already owns this intraminute field. It
            # must never be confused with the previous completed session.
            "previous_close": [103.0, 105.0, 106.0],
            "volume_shares": [3.0, 7.0, 11.0],
            "source_volume_unit_valid": [True, True, True],
...
def test_developing_daily_candle_features_update_each_minute_without_future_leakage() -> (
    ...
    changed_future = add_developing_daily_candle_features(
        _developing_current_session(future_high=200.0), previous
    )

    ordinary_values = ordinary.select(
        MINUTE_DEVELOPING_CANDLE_FEATURE_COLUMNS
    ).to_numpy()
    future_values = changed_future.select(
        MINUTE_DEVELOPING_CANDLE_FEATURE_COLUMNS
    ).to_numpy()
    np.testing.assert_allclose(ordinary_values[:2], future_values[:2])
    assert ordinary["max_logret_1d"][0] == pytest.approx(math.log(105.0 / 104.0))
    assert ordinary["max_logret_1d"][1] == pytest.approx(math.log(107.0 / 104.0))
    assert ordinary["close_logret_1d"][0] == pytest.approx(math.log(105.0 / 103.0))
    assert ordinary["close_logret_1d"][1] == pytest.approx(math.log(106.0 / 103.0))
    assert ordinary["trading_volume_logret_1d"][0] == pytest.approx(
        math.log(3.0 / 30.0)
    )
    assert ordinary["trading_volume_logret_1d"][1] == pytest.approx(
        math.log(10.0 / 30.0)
    )
    assert ordinary["open_logret_1d"].n_unique() == 1
    assert ordinary["previous_close"].to_list() == [103.0, 105.0, 106.0]
    assert ordinary["feature_valid"].to_list() == [True, True, True]


def test_developing_daily_candle_feature_mask_fails_closed_after_bad_prefix() -> None:
    current = _developing_current_session().with_columns(
        pl.when(pl.int_range(pl.len()) == 1)
        .then(False)
        .otherwise(pl.col("source_volume_unit_valid"))
        .alias("source_volume_unit_valid")
    )
    output = add_developing_daily_candle_features(
        current, _completed_previous_minute_session()
    )
    assert output["feature_valid"].to_list() == [True, False, False]


def test_minute_complete_daily_features_split_into_15_dynamic_and_84_context() -> None:
    config = load_config(
        Path(__file__).parents[1] / "configs/markets/tw_minute_dual_5090.yaml"
    )
    context = minute_static_daily_context_feature_names(config.data.feature_include)
    assert len(MINUTE_DEVELOPING_CANDLE_FEATURE_COLUMNS) == 15
    assert len(context) == 84
    assert set(context).isdisjoint(MINUTE_DEVELOPING_CANDLE_FEATURE_COLUMNS)
    assert set(context) | set(MINUTE_DEVELOPING_CANDLE_FEATURE_COLUMNS) == set(
        config.data.feature_include
    )
...
def test_minute_daily_context_uses_prior_complete_session_and_current_open(
    ...
    cache = tmp_path / "panel_cache_v2"
    generation = cache / "generations" / "g1"
    generation.mkdir(parents=True)
    symbols = ["2330", "0050"]  # Exercise non-sorted panel symbol order.
    feature_names = ["daily_a", "daily_b"]
    (generation / "symbols.json").write_text(json.dumps(symbols), encoding="utf-8")
    (generation / "feature_names.json").write_text(
    ...
    context = _load_minute_daily_feature_context(
        meta,
    ...
    # 2026-01-05 sees only the completed 2026-01-02 daily vector.  Symbol
    # order is realigned to the minute universe, while the opening gap uses
    # the already-observed 2026-01-05 open.
    np.testing.assert_allclose(first[:, 0], [21.0, 11.0])
    np.testing.assert_allclose(first[:, 2], [20.0, 10.0])
    np.testing.assert_allclose(first[:, 1], np.log([55.0 / 50.0, 110.0 / 100.0]))
    assert history.shape == (2, 2, 3)
    np.testing.assert_allclose(history[:, :, 0], [[21.0, 11.0], [41.0, 31.0]])
    np.testing.assert_allclose(history[:, :, 2], [[20.0, 10.0], [40.0, 30.0]])
    np.testing.assert_allclose(
        history[:, :, 1],
        [
            np.log([55.0 / 50.0, 110.0 / 100.0]),
            np.log([60.0 / 55.0, 121.0 / 110.0]),
        ],
    )
...
def test_minute_executor_trades_only_target_delta_and_forces_flat() -> None:
    ...
    assert torch.count_nonzero(first.state.shares) == 1
    assert torch.count_nonzero(second.state.shares) == 1
    closed = force_close_minute_session(
        second.state,
...
def test_stateful_proximal_allocator_preserves_exact_no_trade_region() -> None:
    previous = torch.tensor([[0.5, -0.5]], requires_grad=True)
    requested = torch.tensor([[0.5004, -0.4996]], requires_grad=True)
    ...
    torch.testing.assert_close(requested.grad, torch.zeros_like(requested.grad))

    initial = _stateful_proximal_target_weights(
        requested.detach(),
        torch.zeros_like(previous.detach()),
        buy_fee_rates=torch.tensor([0.001, 0.001]),
        sell_fee_rates=torch.tensor([0.002, 0.002]),
        cost_multiplier=1.0,
        long_only=False,
...
def test_recurrent_minute_chunks_keep_future_cost_gradient_to_earlier_target() -> None:
    ...
    initial = initialize_minute_execution_state(
        num_symbols=2,
        config=config,
        device=torch.device("cpu"),
        batch_size=1,
    )
    first_target = torch.tensor([[[0.7, 0.3]]], requires_grad=True)
    shared_market = {
        "execution_masks": torch.ones(1, 1, 2, dtype=torch.bool),
        "execution_opens": torch.tensor([[[100.0, 100.0]]]),
        "exit_closes": torch.tensor([[[100.0, 100.0]]]),
        "future_volumes": torch.full((1, 1, 2), 1_000_000.0),
        "allow_new_entries": torch.tensor([True]),
    }
    first = _execute_minute_chunk(
        initial.cash,
        initial.shares,
        initial.last_prices,
        initial.equity,
    ...
    future_only_loss = -second[4].sum()
    gradient = torch.autograd.grad(future_only_loss, first_target)[0]
    assert bool(torch.isfinite(gradient).all())
    assert float(gradient.abs().sum()) > 0.0


def test_run_day_batch_full_day_credit_changes_earlier_chunk_gradient() -> None:
    class TwoDecisionPolicy(torch.nn.Module):
        def __init__(self) -> None:
            super().__init__()
            self.first_logit = torch.nn.Parameter(torch.tensor(0.7))
            self.second_logit = torch.nn.Parameter(torch.tensor(-0.7))

        def targets(self, feature_slabs: torch.Tensor) -> torch.Tensor:
            ...
            first_weight = torch.sigmoid(active_logit)
            return torch.stack((first_weight, 1.0 - first_weight), dim=-1)

    config = load_config(Path("configs/markets/tw_minute.yaml"))
    config.training.lookback = 1
    config.training.non_blocking_transfer = False
    config.training.minute_stateful_proximal_allocator = False
    ...
    execution_prices = np.zeros((270, 2), dtype=np.float32)
    execution_prices[:2] = 100.0
    ...
    day = MinuteDayPanel(
        ...
        execution_open=execution_prices,
        exit_close=execution_prices,
    ...
    def gradient(*, full_day: bool, checkpoint: bool = False) -> torch.Tensor:
        policy = TwoDecisionPolicy()
    ...
    full_gradient = gradient(full_day=True)
    checkpointed_full_gradient = gradient(full_day=True, checkpoint=True)
    truncated_gradient = gradient(full_day=False)
...
def test_financial_transformer_fuses_daily_context_without_bar_repetition() -> None:
    ...
    fin.use_market_tokens = False
    fin.d_model = 8
    fin.temporal_layers = 1
    fin.temporal_heads = 1
    fin.temporal_ffn_mult = 1
    fin.head_hidden_dim = 8
    fin.checkpoint_blocks = False
    fin.daily_context_lookback = 2
    ...
    output = model.forward_from_batched_panel_slabs_with_daily_context(
        slabs,
        daily,
        mask,
        return_aux=False,
    )
    encoded_daily = model.encode_daily_context_history(daily)
    preencoded_output = (
        model.forward_from_batched_panel_slabs_with_encoded_daily_context(
            slabs,
            encoded_daily,
            mask,
            return_aux=False,
    ...
    torch.testing.assert_close(preencoded_output, output)
    output.square().sum().backward()
    assert model.daily_context_gate is not None
    assert model.daily_context_gate.grad is not None
    assert bool(torch.isfinite(model.daily_context_gate.grad).all())
    assert len(model.daily_context_temporal_blocks) == 1
    daily_grad = next(model.daily_context_temporal_blocks.parameters()).grad
    assert daily_grad is not None
    assert bool(torch.isfinite(daily_grad).all())
...
def test_dual_5090_minute_config_is_within_fold_ddp() -> None:
    ...
    assert config.training.minute_full_day_credit_assignment is True
    assert config.training.minute_full_day_activation_checkpoint is True
    ...
    assert config.training.financial_transformer.daily_context_layers == 2
    ...
    assert config.data.feature_include[-1] == MINUTE_DAILY_OPEN_GAP_FEATURE
...
def test_dual_5090_minute_trading_contract_matches_daily_day_trade() -> None:
    ...
    assert minute.trading.buy_fee_rate == pytest.approx(0.000285)
    assert minute.trading.sell_fee_rate == pytest.approx(0.001785)
    buy_rates, sell_rates = effective_fee_rate_vectors(
        ["2330", "0050"],
        "tw_day_trade",
        fee_schedule=_fee_schedule(minute),
    )
