...
def test_return_label_uses_adjclose_but_execution_price_uses_close() -> None:
    ...
    assert math.isclose(float(prepared.loc[0, "return_1d"]), math.log(105.0 / 100.0), rel_tol=1e-7)
    assert math.isclose(float(prepared.loc[1, "return_1d"]), 0.0, abs_tol=1e-7)
    assert math.isclose(float(prepared.loc[1, "close_logret_1d"]), math.log(110.0 / 100.0), rel_tol=1e-7)
